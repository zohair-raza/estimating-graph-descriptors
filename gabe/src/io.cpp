...
{
	long double * gCnt = new long double[6];
    //std::fill_n(gCnt, 6, 0.0);
    ...
    clock_t begin = clock();
	MPI_Reduce(gCnt, nullptr, 6, MPI_LONG_DOUBLE, MPI_SUM, MPI_MASTER, MPI_COMM_WORLD);
    VID maxVId;
...
bool MPIIO::recvCnt(VID maxVId, std::vector<long double> &gCntV)
{
    ...
    clock_t begin = clock();
	MPI_Reduce(MPI_IN_PLACE, gCnt, 6, MPI_LONG_DOUBLE, MPI_SUM, MPI_MASTER, MPI_COMM_WORLD);
    MPI_Bcast(&maxVId, 1, MPI_UNSIGNED, MPI_MASTER, MPI_COMM_WORLD);
    ioCPUTime += double(clock() - begin);
